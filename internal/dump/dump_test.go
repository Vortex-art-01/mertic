...
func newTestFile(t *testing.T, path string) (*File, *repository.MemStorage) {
	t.Helper()

	repo := repository.NewMemStorage()
	return New(path, repo, slog.New(slog.DiscardHandler)), repo
}

func TestSaveAndLoad(t *testing.T) {
	path := filepath.Join(t.TempDir(), "metrics-db.json")

	saver, source := newTestFile(t, path)
	...
	loader, restored := newTestFile(t, path)
...
func TestLoadReplacesCounter(t *testing.T) {
	...
	loader, repo := newTestFile(t, path)
...
func TestLoadMissingFile(t *testing.T) {
	loader, repo := newTestFile(t, filepath.Join(t.TempDir(), "missing.json"))
